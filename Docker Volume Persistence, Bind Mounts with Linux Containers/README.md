...
```sh
docker exec -it alpine_with_bind_mount sh -c "echo 'Hello, Aditya!' > /data/testfile.txt"
```
#### 🔍 What Happened?
- The command executed a shell inside the running container.
- It created a file `testfile.txt` inside `/data` and wrote **"Hello, Aditya!"** into it.
