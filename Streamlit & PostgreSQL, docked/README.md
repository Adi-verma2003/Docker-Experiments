...
```sh
docker run --name my_postgres_container --network my_postgres_network -e POSTGRES_USER=aditya -e POSTGRES_PASSWORD=secret -e POSTGRES_DB=testdb -p 5432:5432 -d postgres
```
...
```sh
docker exec -it my_postgres_container psql -U aditya -d testdb
```
